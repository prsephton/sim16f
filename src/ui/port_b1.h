	...
	class PortB1: public CairoDrawing {
	  ...
	  public:
		...
		virtual ~PortB1() {
			...
			DeviceEvent<Wire>::unsubscribe<PortB1>(this, &PortB1::on_wire_change, &DataBus);
			DeviceEvent<BasicPort>::unsubscribe<PortB1>(this, &PortB1::on_port_change, &p1);
		}
			...
			m_cpu(a_cpu), m_refGlade(a_refGlade)
		{
			...
			m_components["Out_en"]= new ConnectionDiagram(Out_en.rd(), 315.0, 240.0, m_area);
			m_components["dMUX"]= new ConnectionDiagram(dMux.rd(), 305.0, 125.0, m_area);
			m_components["USART.out"]= new ConnectionDiagram(p1.USART_Data_Out(), 100.0, 115.0, m_area);
