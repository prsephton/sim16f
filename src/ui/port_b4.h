...
namespace app {
	...
	class PortB4: public CairoDrawing {
		CPU_DATA &m_cpu;
	  ...
	  public:
		...
		void draw_pin_wire() {
			...
			wire.add(WireDiagram::pt(500, 270).first().join());
		...
		virtual ~PortB4() {
			...
			DeviceEvent<Wire>::unsubscribe<PortB4>(this, &PortB4::on_wire_change, &DataBus);
			DeviceEvent<BasicPort>::unsubscribe<PortB4>(this, &PortB4::on_port_change, &p4);
		}

		PortB4(CPU_DATA &a_cpu, const Glib::RefPtr<Gtk::Builder>& a_refGlade):
			...
			m_cpu(a_cpu), m_refGlade(a_refGlade)
		{
			...
			OrGate &Out_en = dynamic_cast<OrGate &>(*c["OR(TrisLatch.Q, LVP)"]);

			Wire &REC_wire = dynamic_cast<Wire &> (*(c["PGM_RECWire"]));
			...
			m_components["Pin"]  = new PinDiagram(p4.pin(), 530, 105, 0, 1, m_area);
			...
			m_components["RBPU"]= new ConnectionDiagram(p4.RBPU(), 100.0, 50.0, m_area);
			...
			m_components["PGM"] = new WireDiagram(REC_wire, 105, 270, m_area);
