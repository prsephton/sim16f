...
namespace app {
	...
	class PortB7: public CairoDrawing {
		CPU_DATA &m_cpu;
	  ...
	  public:
		...
		void draw_pin_wire() {
			...
			wire.add(WireDiagram::pt(505, 265).first().join());
			wire.add(WireDiagram::pt(480, 265));
			...
			wire.add(WireDiagram::pt(350, 300));
		}
		...
		void draw_t1oscen() {
			...
			conn.add(ConnectionDiagram::pt(235, 35).first().join());
			conn.add(ConnectionDiagram::pt(235, 5));

			// connect to SPROG_en And Gate
			conn.add(ConnectionDiagram::pt(315,  35).first().join());
			conn.add(ConnectionDiagram::pt(315, -25));
			conn.add(ConnectionDiagram::pt(305, -25).invert());
		...
		void draw_t1osc() {
			...
			conn.add(ConnectionDiagram::pt(220,   0));
			conn.add(ConnectionDiagram::text(0, -2, "To RB7"));
		}
		...
		virtual ~PortB7() {
			auto &p7 = dynamic_cast<PortB_RB7 &>(*(m_cpu.portb.RB[7]));
			auto &c = p7.components();
			Wire &DataBus = dynamic_cast<Wire &> (*(c["Data Bus"]));

			DeviceEvent<Wire>::unsubscribe<PortB7>(this, &PortB7::on_wire_change, &DataBus);
			DeviceEvent<BasicPort>::unsubscribe<PortB7>(this, &PortB7::on_port_change, &p7);
		}

		PortB7(CPU_DATA &a_cpu, const Glib::RefPtr<Gtk::Builder>& a_refGlade):
			CairoDrawing(Glib::RefPtr<Gtk::DrawingArea>::cast_dynamic(a_refGlade->get_object("dwg_RB7"))),
...
//			AndGate &SPROG_en = dynamic_cast<AndGate &> (*(c["AND(iT1OSCEN, Trigger)"]));
			...
			Wire &SPROG = dynamic_cast<Wire &> (*(c["SPROG"]));
			...
			DeviceEvent<Wire>::subscribe<PortB7>(this, &PortB7::on_wire_change, &DataBus);
			DeviceEvent<BasicPort>::subscribe<PortB7>(this, &PortB7::on_port_change, &p7);
			...
			m_components["Pin"]  = new PinDiagram(p7.pin(), 550, 105, 0, 1, m_area);
			...
			m_components["T1OSCEN"]  = new ConnectionDiagram(p7.T1OSCEN(), 100, 300, m_area);
			m_components["OutBuf"]  = new ConnectionDiagram(Out_Buffer.rd(), 500, 350, m_area);

			m_components["T1OSC_Tristate"]  = new TristateDiagram(T1OSC, false, 350, 300, m_area);
			m_components["T1OSC"]  = new ConnectionDiagram(p7.T1OSC(), 100, 300, m_area);

			m_components["Q1"]  = new ConnectionDiagram(p7.Q1(), 560, 445, m_area);
			m_components["Q3"]  = new ConnectionDiagram(p7.Q3(), 560, 530, m_area);
